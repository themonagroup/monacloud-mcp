...
import { completeToolMetadata, createMonaPayClient, importedMonaPayTools, normalizeImportedResult, toolAnnotationsForName } from './monapay.js';
...
export function createServer(dependencies: ServerDependencies = {}): McpServer {
  ...
  const server = new McpServer(
    ...
    { instructions: `Tên miền khi người dùng CHƯA có tài khoản: cloud_domain_search (không cần login) → cloud_domain_reserve (email + sđt) → đưa QR + claim_url (người dùng chỉ quét QR rồi đăng nhập một bước, không phải đăng ký trước); có tài khoản rồi thì cloud_domain_claim/cloud_domain_buy. Ví thiếu tiền: AI tự gọi cloud_topup, in nguyên khối qr_ascii (QR VietQR) cho người dùng quét bằng app ngân hàng ngay trong terminal, rồi cloud_topup_status tới khi paid. Dùng cloud_* cho tài khoản, ví và hạ tầng; monapay_* cho thu tiền; mail_* để gửi email giao dịch (MONA Mail); agent_* cho catalog. Không bao giờ yêu cầu mật khẩu sản phẩm. Đọc → ước tính → hỏi duyệt nếu chưa được duyệt → làm. Dừng chờ người dùng ở: quét QR nạp, duyệt chi phí, thêm DNS, OTP hoặc KYC.\n${ENTITY}\n${APP_FLOW}` },
  );

  server.registerTool('cloud_whoami', {
    annotations: toolAnnotationsForName('cloud_whoami'),
  ...
  server.registerTool('cloud_balance', {
    annotations: toolAnnotationsForName('cloud_balance'),
  ...
  server.registerTool('cloud_ledger', {
    annotations: toolAnnotationsForName('cloud_ledger'),
  ...
  server.registerTool('cloud_topup', {
    annotations: toolAnnotationsForName('cloud_topup'),
    title: 'Nạp ví bằng VietQR ngay trong terminal',
    description: 'AI tự tạo yêu cầu nạp ví và nhận QR VietQR: qr_ascii (khối đầy, in thẳng trong Claude Code/Codex/Gemini; nền sáng dùng qr_ascii_light), qr_file (PNG trên máy), qr_url (ảnh). Người dùng chỉ quét bằng app ngân hàng. Tiền vào tự cộng ví; xác nhận bằng cloud_topup_status.',
    inputSchema: {
  ...
  server.registerTool('cloud_topup_status', {
    annotations: toolAnnotationsForName('cloud_topup_status'),
  ...
  server.registerTool('cloud_usage', {
    annotations: toolAnnotationsForName('cloud_usage'),
  ...
  server.registerTool('cloud_services', {
    annotations: toolAnnotationsForName('cloud_services'),
  ...
  server.registerTool('cloud_budget_set', {
    annotations: toolAnnotationsForName('cloud_budget_set'),
  ...
  server.registerTool('cloud_budget_get', {
    annotations: toolAnnotationsForName('cloud_budget_get'),
  ...
  server.registerTool('cloud_token_limit', {
    annotations: toolAnnotationsForName('cloud_token_limit'),
  ...
  server.registerTool('cloud_open_console', {
    annotations: toolAnnotationsForName('cloud_open_console'),
  ...
  server.registerTool('monapay_link', {
    annotations: toolAnnotationsForName('monapay_link'),
  ...
  server.registerTool('cloud_link', {
    annotations: toolAnnotationsForName('cloud_link'),
  ...
  server.registerTool('vibecloud_link', {
    annotations: toolAnnotationsForName('vibecloud_link'),
  ...
  server.registerTool('cloud_vps_create', {
    annotations: toolAnnotationsForName('cloud_vps_create'),
  ...
  server.registerTool('vibecloud_create_vps', {
    annotations: toolAnnotationsForName('vibecloud_create_vps'),
  ...
  server.registerTool('cloud_db_create', {
    annotations: toolAnnotationsForName('cloud_db_create'),
  ...
  server.registerTool('vibecloud_create_database', {
    annotations: toolAnnotationsForName('vibecloud_create_database'),
  ...
  server.registerTool('cloud_job_status', {
    annotations: toolAnnotationsForName('cloud_job_status'),
  ...
  server.registerTool('vibecloud_job_status', {
    annotations: toolAnnotationsForName('vibecloud_job_status'),
  ...
  server.registerTool('cloud_services_list', {
    annotations: toolAnnotationsForName('cloud_services_list'),
  ...
  server.registerTool('vibecloud_list_services', {
    annotations: toolAnnotationsForName('vibecloud_list_services'),
  ...
  for (const action of ['start', 'stop', 'rebuild'] as const) {
    ...
    server.registerTool(name, {
      annotations: toolAnnotationsForName(name),
    ...
    server.registerTool(`vibecloud_${action}`, {
      annotations: toolAnnotationsForName(`vibecloud_${action}`),
  ...
  server.registerTool('cloud_packages', {
    annotations: toolAnnotationsForName('cloud_packages'),
  ...
  server.registerTool('vibecloud_packages', {
    annotations: toolAnnotationsForName('vibecloud_packages'),
  ...
  server.registerTool('cloud_agent_deploy', {
    annotations: toolAnnotationsForName('cloud_agent_deploy'),
  ...
  server.registerTool('vibecloud_agent_deploy', {
    annotations: toolAnnotationsForName('vibecloud_agent_deploy'),
  ...
  server.registerTool('agent_templates_list', {
    annotations: toolAnnotationsForName('agent_templates_list'),
  ...
  server.registerTool('agent_templates_get', {
    annotations: toolAnnotationsForName('agent_templates_get'),
  ...
  server.registerTool('agent_deploy', {
    annotations: toolAnnotationsForName('agent_deploy'),
  ...
  for (const [name, tool] of Object.entries(imported)) {
    ...
    register(name, {
      ...completeToolMetadata(name, tool.title, tool.annotations),
  ...
  // Complete metadata after every native registrar has run. This keeps schemas,
  // handlers and registration order untouched while covering aliases mechanically.
  const registeredTools = (server as unknown as {
    _registeredTools: Record<string, { title?: unknown; annotations?: unknown }>;
  })._registeredTools;
  for (const [name, tool] of Object.entries(registeredTools)) {
    Object.assign(tool, completeToolMetadata(name, tool.title, tool.annotations));
  }
