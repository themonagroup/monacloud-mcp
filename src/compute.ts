...
export function registerComputeTools(server: McpServer, clients: CloudClients) {
  ...
  register('cloud_app_domain_add', appNotice + 'Thêm domain human ĐÃ có, trả hướng dẫn CNAME từ API; chờ DNS trước kiểm HTTPS. Human chưa có domain → dùng cloud_domain_search/cloud_domain_reserve/cloud_domain_buy rồi cloud_domain_attach (mua .vn/.com bằng VND ngay trong phiên). / Attach an existing custom domain; to buy one use cloud_domain_*.', appId.extend({ host: domain }), ({ app_id, host, sandbox }) => compute(`/api/apps/${encodeURIComponent(app_id)}/domains`, { method: 'POST', body: { host } }, sandbox));
